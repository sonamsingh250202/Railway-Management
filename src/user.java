...
class user
{
    ...
    void inputreserve() throws Exception
    {
        ...
        else
        {
            ...
            chart(pname,page,pgen,coach,tnum);
        }
    }
...
//            String str8 = dF.format(r2.getDate(9));
            ...
            System.out.println("Booked on : "+str7);
            System.out.println("***************************************************************************************");
        }
        catch (Exception e)
        {
    ...
    void chart(String pname[],int page[], String pgen[], String coach,int tnum)
    {
        try
        {
            int pnr = 0;
            ...
            for(int i=0;i<seat;i++)
            {
                ...
                ResultSet r1=s1.executeQuery("select * from train where tnum='"+tnum+"'");
                r1.first();
                PreparedStatement st=c.prepareStatement("insert into chart (pnr,name,age,gender,seatno,coach,status,timestamp,tnum) values(?,?,?,?,?,?,?,?,?)");
                st.setInt(1, pnr+1);
                st.setString(2,pname[i]);
                st.setInt(3,page[i]);
                st.setString(4,pgen[i]);
                st.setInt(5,seatNo+1);
    ...
    void setw(int tnum, String str1, int seats,String str10,String str11, int fAc,int sAc,int tAc,int sc, String str7,String str9, int width)
    {
    ...
    void enquiry() throws Exception
    {
        ...
        ResultSet r=st.executeQuery("select * from train where bp='"+from+"' and dp='"+to+"'");
        while(r.next())
        {
            //tnum*****tname****seats******bp*******dp********fAC****sAC*****tAC******sc***doj*******dtime***atime*****sno
            setw(r.getInt(1),r.getString(2),r.getInt(3),r.getString(4),r.getString(5),r.getInt(6),r.getInt(7),r.getInt(8),r.getInt(9),r.getString(11),r.getString(12),15);
